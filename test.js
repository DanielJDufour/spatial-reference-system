...
test("SRS({ code, wkt })", ({ eq }) => {
  ...
  // higher precision wkt from https://epsg.io/3857.esriwkt
  eq(
    srs.wkt.esri,
    'PROJCS["WGS_1984_Web_Mercator_Auxiliary_Sphere",GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.017453292519943295]],PROJECTION["Mercator_Auxiliary_Sphere"],PARAMETER["Auxiliary_Sphere_Type",0.0],PARAMETER["Central_Meridian",0.0],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Standard_Parallel_1",0.0],UNIT["Meter",1.0]]'
  );
});
...
test(`SRS("3857")`, ({ eq }) => {
  ...
  eq(
    srs.wkt.ogc,
    'PROJCS["WGS 84 / Pseudo-Mercator",GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4326"]],PROJECTION["Mercator_1SP"],PARAMETER["central_meridian",0],PARAMETER["false_easting",0],PARAMETER["false_northing",0],PARAMETER["scale_factor",1],UNIT["metre",1,AUTHORITY["EPSG","9001"]],AXIS["X",EAST],AXIS["Y",NORTH],EXTENSION["PROJ4","+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs"],AUTHORITY["EPSG","3857"]]'
  );
  eq(
    srs.proj4,
    "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs"
...
test("SRS(proj4 string)", ({ eq }) => {
  ...
  eq(
    srs.wkt.ogc,
    'PROJCS["NAD83 / UTM zone 16N",GEOGCS["NAD83",DATUM["North_American_Datum_1983",SPHEROID["GRS 1980",6378137,298.257222101,AUTHORITY["EPSG","7019"]],TOWGS84[0,0,0,0,0,0,0],AUTHORITY["EPSG","6269"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4269"]],PROJECTION["Transverse_Mercator"],PARAMETER["central_meridian",-87],PARAMETER["false_easting",500000],PARAMETER["false_northing",0],PARAMETER["latitude_of_origin",0],PARAMETER["scale_factor",0.9996],UNIT["metre",1,AUTHORITY["EPSG","9001"]],AXIS["Easting",EAST],AXIS["Northing",NORTH],AUTHORITY["EPSG","26916"]]'
  );
  eq(srs.proj4, str);
});
...
test("equivalent two 32767", ({ eq }) => {
  eq(equivalent(new SRS(32767), new SRS(32767)), false);
});
...
test("custom SRS(32767)", ({ eq }) => {
  const srs = SRS(32767);
  eq(srs.id, undefined);
  eq(srs.code, 32767);
});

test("not equivalent(32767, 32767)", ({ eq }) => {
  eq(equivalent(32767, 32767, { debug: false }), false);
});
...
test("SRS(obj)", ({ eq }) => {
  ...
  const srs = new SRS(obj);
  eq(srs.code, 3857);
  eq(srs.wkt.ogc, obj.wkt.ogc);
  eq(srs.proj4, obj.proj4);
  eq(srs.prj, obj.wkt.ogc);
});
